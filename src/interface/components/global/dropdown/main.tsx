...
const Dropdown: React.FC<dropdownProps> = ({options, selectedOption, setSelectedOption, width, colorBall }) => {
    ...
    return (
      <S.DropdownContainer data-width={ width ? width : null }>
        ...
        <S.Ball $color={colorBall} /> {selectedOption} <S.Icon src={arrowDown} alt="Ícone de flecha para baixo"/>
        </S.DropdownHeader>
        {isOpen && (
          <S.DropdownList>
            {options.map((option) => (
