...
const AddOrRemove: React.FC<addOrRemoveProps> = ({ quantity, handleQuantity, loading, minQuantity }) => {
  const increase = () => handleQuantity(quantity + 1);
  const decrease = () => handleQuantity(quantity - 1);

  return (
    <S.CounterWrapper>
      <S.ButtonBackground onClick={decrease} disabled={minQuantity ? quantity === minQuantity : quantity === 1}>
